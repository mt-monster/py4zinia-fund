    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if self.config.get('llm_api_key'):
            self.llm_client = OpenAI(
                base_url=self.config.get('llm_base_url', 'https://ark.cn-beijing.volces.com/api/v3'),
        ...
        for step in plan:
            ...
            if tool_name and tool_name in self.tools:
                try:
                    ...
                    # 更新上下文
                    context.update(step_result if isinstance(step_result, dict) else {'result': step_result})
