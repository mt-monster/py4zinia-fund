...
class RouteRegistry:
    ...
    def register_route_module(
        self, module_name: str, module_path: str, **dependencies
    ) -> bool:
        """
        注册单个路由模块
    ...
    def register_all_routes(
        self, routes_dir: str, dependencies: Dict[str, Any]
    ) -> Dict[str, bool]:
        """
        注册所有路由模块
...
def register_routes_automatically(
    app: Flask, routes_directory: str, dependencies: Dict[str, Any] = None
) -> Dict[str, bool]:
    """
    自动注册路由的便捷函数
...
def print_route_summary(router: RouteRegistry):
    ...
    success_count = sum(
        1 for info in registered_routes.values() if info["status"] == "registered"
    )
